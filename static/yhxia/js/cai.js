	  ...
	  xhr.onreadystatechange =function(){
	   if((this.readyState==4)&&(this.status==200)){ 
        ...
        if (json.status==0){
        	cont.innerHTML='';
          tsxx.innerHTML='';
          tsxx.innerHTML=json.data+'---->开始采集下个【'+overtx+'】分类<img src="'+ pic +'"/>';
          ...
          tsxx.innerHTML='【'+tx+'】分类采集完成！即将添加！<img src="'+ pic +'"/>';
...
function zhunbei(){
  ...
  $.ajax({
   ...
   if(msg.status==8) {
   tx = msg.data;
   tsxx.innerHTML='开始采集【'+tx+'】分类<img src="'+ pic +'"/>';}},
   dataType:'json'
  });
  setTimeout(cai('start'),3000);
}
function begin(){
tsxx.innerHTML='<font color="green">采集准备中<img src="'+ pic +'"/></font>';
cont.innerHTML = '';
setTimeout("zhunbei()",1000);
}
function add(name){
  tsxx.innerHTML='正在添加到【'+name+'】分类<img src="'+ pic +'"/>';
...
function addok(){
	  ...
	  xhr.onreadystatechange =function(){
	   if((this.readyState==4)&&(this.status==200)){ 
        var rz =this.responseText;
        var json = eval('('+rz+')');
        tsxx.innerHTML = '【'+json.msg.cname+'】分类宝贝添加完成----->【'+json.msg.nextcate+'】分类采集中<img src="'+ pic +'"/>'; 
        cont.innerHTML = json.data;
        setTimeout("cai('ff')",1000);
	   }else{
...
$('#bd span:last-child').css('display','none');
   ...
   $('#zhe').css({'display':'block','left':windowWidth/4,'top':windowHeight/10});
});
      ...
      var inputs = $('input');
       for(j=0;j<len;j++){
          for (var n = inputs.length - 1; n >= 0; n--) {
            ...
            if(k==temp[j]){$(inputs[n]).attr('checked','checked');}
          }         
       }
      },
   	'json'
