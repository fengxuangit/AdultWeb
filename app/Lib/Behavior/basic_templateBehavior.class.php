    ...
    private function parseTemplateFile($templateFile) {
        ...
        if(''==$templateFile) {
            ...
            if(!file_exists_case($templateFile) && C('DEFAULT_THEME') && C('BASIC_THEME')) {
                ...
                $yh_default_theme = C('BASIC_THEME');
                $group = defined('GROUP_NAME') ? GROUP_NAME . '/' : '';
                $yh_theme_path = TMPL_PATH . $group . $yh_default_theme . '/';
                $templateFile = $yh_theme_path.MODULE_NAME.(defined('GROUP_NAME')?C('TMPL_FILE_DEPR'):'/').ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX');
            }
